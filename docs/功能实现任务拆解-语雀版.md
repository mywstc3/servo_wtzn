...
**工程路径：** `servo_project/`  
**编写日期：** 2026-06-24 · **最近更新：** 2026-07-15 · 文档版本：v3.8-语雀
>
...
**当前固件（2026-07-15）：** `motor.c` 位置模式为「`angle_control_plan` 时间域梯形 → 精调区位置 PD（跟随模式下禁用）→ `plan_speed` → 速度 PI + Stribeck oneshot 前馈 → duty」；高频 GOAL 由 `motor_follow` 识别；STS 经 `sts_mem` 桥接（含连续 GOAL 多圈跟踪）；Flash EPROM 与中位校准已合入；速度观测为 MA+跨距差分+真实Δt（VOFA 验收待做）。多机：助手可通，上位机超时根因未闭环（见 §9）。
...
| **遇到问题** | 多机在 FD/上位机易超时；7/15 助手同命令可通，原厂/自研字节延迟≈10μs vs 12μs |
| **方案改进** | 后期补 REG_WRITE+ACTION、SYNC_READ/WRITE；P0 对照上位机轮询/超时（见交接文档 §8） |
| **验收标准** | 合法帧正确应答；非法帧返回 ERROR 状态字节 |
| **验收效果及问题** | PING/READ/WRITE/RESET/CALIB 实机应答正确；RX discard 已合入。多机：排除字节延迟与舵机不应答，根因倾向上位机侧，未闭环 |
...
| C-02 | 已完成 | `sts_proto` PING/READ/WRITE/RESET/CALIB + RX discard；多机上位机超时见下 |
...
5. **discard 与 ~100°/s 饱和**实机验收（与 P0 分开）
