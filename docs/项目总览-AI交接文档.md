...
> **最后更新**：2026-07-21（多机时序对齐 + 方案 A 成帧即应答 + RETURN_DELAY + 搜 ID 收包策略；§0.5 已同步）  
> **目标产品**：飞特 STS3215c018 磁编码舵机兼容固件  
> **硬件平台**：16999-PS26040802 伺服板（GD32F130F8 + AS5600 + EG2104 H 桥）  
> **代码基准**：分支「将串口接收完一帧…直接应答」@ `32ccd63` + **工作区未提交**：方案 A / RETURN_DELAY / 非本机整帧过滤等协议改动，及 7/21 LA 抓包数据  
...
> **项目名：** STS 舵机固件  
> **进度日期：** 2026-07-21  
> **最后更新：** 2026-07-21（据当日舵机会话与工作区改动覆盖）

### 今日做了什么

- 协议调度方案 A：校验通过后当场 `process` + 回包，去掉「先入帧队列再 pop」一跳（对齐虚拟舵机成帧即发）；不再动已修好的 `bap_uart_send` 帧内节拍。
- LA 抓包 + MATLAB/Python 对比：改板 2.0 单机 vs 原厂；改版 2.0 多机 vs 旧多机 / 原厂多机（`260721-*` CSV + `cleaned/figures`）。
- 实现内存表 `RETURN_DELAY`（0x07）：应答前按档位延时（单位 2μs）；默认调参后 CS→`FF` 空闲可与原厂同量级（约 32μs）。
- 多机搜 ID 仍失败：对照应答帧内容与原厂一致；排查后改收包策略——非本机 ID 先收齐整帧再过滤（取消 ID 字节提前 discard），广播 `0xFE` 一律不应答（对齐虚拟舵机）。

### 完成度 / 结论

- **时序侧已闭环**：改板 2.0 应答字节间隔≈9.9μs，单机 TX→RX 转化约 26μs（原厂约 44μs）；多机抓包亦成对齐全。字节间隙/转化过慢不再能解释「原厂通、自研不通」。
- **RETURN_DELAY 已落地**；过快回包可能干扰 FD 半双工切换的判断得到部分验证与对策。
- **搜 ID 根因未闭环**：嫌疑从「上位机超时/时序」收窄到「多机总线收包失步 / FD 扫 ID 判定」；收包策略已改，**待烧录后用 FD 再验搜 ID**。

### 卡点

- FD 多机搜 ID 仍失败（改收包策略后尚未确认是否恢复）。阻塞程度高；需实机复测，失败则共抓 FD 扫 ID 全过程 vs 原厂。

### 需对外协同

- 对接人：上位机/FD 相关同事 | 事项：自研多机搜 ID 失败、原厂/虚拟可通 | 需求：若固件复测仍失败，共抓扫 ID 命令序列与主机判定逻辑 | P0

### 明日建议（供日报「明日计划」参考）

- **P0：** 烧录验证「整帧再过滤 + 广播不应答」后 FD 搜 ID；目标：能搜到本机 ID 或给出可复现对照抓包。
- **P1：** 仍失败则 LA 对比自研 vs 原厂「FD 搜 ID」全过程（指令/间隔/应答）。
- **P2：** 提交工作区协议 WIP（方案 A、RETURN_DELAY、收包策略）与 7/21 测试数据。

### 相关产物路径（可选）

- `测试数据/改板2.0单机测试数据--260721-100730.csv`
- `测试数据/改版2.0多机测试数据--260721-103751.csv`、`原厂多机测试数据--260721-104538.csv`
- `测试数据/cleaned/figures/`（fix2 vs oem 对比图）
- `servo_project/User/servo/sts_proto.c`、`uart.c`、`sts_mem*.{c,h}`
